...
public class MazeRendererNew : MonoBehaviour
{
    ...
    private void Start()
    {
            ...
            for (int y = 0; y < mazeGenerator.mazeHeight; y++)
            {
                ...
                foreach (Transform child in newCell.transform)
                {
                    if (child.CompareTag("Wall"))
                    {
                        ...
                        child.gameObject.GetComponent<Renderer>().material = MazeGenerator.S.dungeon.wallMaterial;
                    }

                    if (child.CompareTag("Ground"))
                    {
                        child.gameObject.GetComponent<Renderer>().material = MazeGenerator.S.dungeon.floorMaterial;
                    }
                }
        ...
        // Creating Exit
        int exitX = Random.Range(0, mazeGenerator.mazeWidth);
        int exitY = Random.Range(0, mazeGenerator.mazeHeight);
        
        Instantiate(exit, new Vector3(exitX, -0.2f, exitY), Quaternion.identity, transform);
    }
